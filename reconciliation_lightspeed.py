...
# Must match the store's actual local timezone - ValorPay's report times are
# local (e.g. "07/05/2026 22:56" with no offset), while Lightspeed's API
# returns UTC timestamps. If any of your 10 stores are in a different time
# zone than this, that store's matching will be off by the zone difference.
...
def _extract_local_timestamp(sale):
    """
...
def fetch_card_sales(config, store_key, start_date, end_date, employees):
    ...
    for sale in all_raw_sales:
        ...
        timestamp = _extract_local_timestamp(sale)
